...
3. **`buildContainer()` + `renderPanes(container, athletes)`** — build once, re-render often. The container holds two tab panes (*By Rank*, *By Start Order*) and a paging control; `renderPanes` slices the athletes list to the current page window and regenerates each pane's `<table>`. Tab-switch and paging listeners are attached once and survive re-renders because only the panes' children are replaced. Paging state (`__tmPageSize`, `__tmPage`, `__tmAthletes`) lives on the container element so handlers can re-render without re-extracting.
